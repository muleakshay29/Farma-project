...
export class AddCommonMasterComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if(this.cmID == "" || this.cmID == null) {
      ...
      this.masterservice.addCommonMaster(formData)
      .subscribe( (data) => {
        if(data > 0) {
          ...
          this.alertService.openSnackBar('Error adding record. Please try again.');
        }
      });
    } else {
      const formData = {
      ...
      this.masterservice.updateCommonMaster(formData)
        .subscribe((data) => {
          if(data > 0) {
  ...
  fetchCommonMasterDetails() {
    this.masterservice.fetchCommonMasterDetails(this.cmID)
      .subscribe( (details) => {
