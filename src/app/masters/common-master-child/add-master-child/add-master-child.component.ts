...
export class AddMasterChildComponent implements OnInit {
  ...
  onSubmit() {
    if (this.cmcID == "" || this.cmcID == null) {
      ...
      this.masterservice.addCommonMasterChild(formData)
        .subscribe((data) => {
          if(data > 0) {
      ...
      this.masterservice.updateCommonMasterChild(formData)
        .subscribe((data) => {
          if(data > 0) {
  ...
  fetchCommonMasterChildDetails() {
    this.masterservice.fetchCommonMasterChildDetails(this.cmcID)
      .subscribe((details) => {
  ...
  fetchCommonMaster() {
    this.masterservice.fetchCommonMaster()
